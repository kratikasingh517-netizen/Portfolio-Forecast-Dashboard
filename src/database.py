...
def save_results_to_supabase(result: dict[str, Any]) -> None:
    ...
    try:
        response = supabase.table(SUPABASE_TABLE_NAME).insert(rows).execute()
        logger.info(f"Successfully saved {len(rows)} predictions to Supabase")
    except Exception as e:
        error_msg = str(e)
        if "PGRST205" in error_msg or "Could not find the table" in error_msg:
            raise ValueError(
                f"Table '{SUPABASE_TABLE_NAME}' does not exist in Supabase. "
                "Please run the SQL schema from 'schema.sql' file in your Supabase SQL Editor. "
                "See README.md Step 3 for detailed instructions."
            ) from e
        raise ValueError(f"Failed to insert data into Supabase: {error_msg}") from e
